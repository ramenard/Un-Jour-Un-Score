...
export class ObtainedBadgesService {
  ...
  public async create(
    createObtainedBadgeDto: CreateObtainedBadgeDto,
  ): Promise<void> {
    const user = await this.usersService.findOneById(
      createObtainedBadgeDto.userId,
    );
    const badge = await this.badgesService.findOne(
      createObtainedBadgeDto.badgeId,
    );
  ...
  public async findAll(userId?: string): Promise<ObtainedBadge[]> {
    const queryBuilder = this.obtainedBadgeRepository
      .createQueryBuilder()
  ...
  public async findOne(id: string): Promise<ObtainedBadge> {
    const obtainedBadge = await this.obtainedBadgeRepository.findOne({
      where: { id: id },
    });
    if (!obtainedBadge) {
